      ...
      class << self
        ...
        def scan_code_pay opt
          ...
          ext_opt = opt.slice(
            ...
            :callback_url, :jspay_flag, :goods_tag, :limit_pay, :shop_no, :pos_no, :attach, :app_id)

          options = options.merge ext_opt
          puts signature(options)
          http_post(Config::LEPOS_PAY_GATEWAY_URL, signature(options))
        end
        ...
        def signature options
          LeshuaPay::Common::Sign.signature options
        end
      end
    end
  end
